...
# Define the completion prompt template

# Specify the file to read
file_number = 2  # We'll only process 1.txt
...
data = {
    ...
    "top_p": 1.0,
    "stop": "###"  # Tell the model to stop generating when it encounters ###
}
...
response_text = response.text[:-3]  # Get the raw response as a string
generated_text = response.json().get("choices", [{}])[0].get("text", "")  # Extract the generated text
parsed_generated_json = json.loads(generated_text)  # Parse the generated text as JSON
pretty_json = json.dumps(parsed_generated_json, indent=4)
print(pretty_json)
